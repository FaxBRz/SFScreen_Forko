...
const directPeerConfiguration: RTCConfiguration = { iceServers: [], iceTransportPolicy: 'all', bundlePolicy: 'max-bundle' };
...
export const App = (): ReactElement => {
  ...
  const closeSession = (): void => {
    ...
    setRole(undefined); setSecurityCode(undefined); setConnected(false); setError(undefined); setStatus('Sessão encerrada.');
  };

  const preparePeer = (): { peer: RTCPeerConnection; candidates: RTCIceCandidateInit[] } => {
    const candidates: RTCIceCandidateInit[] = [];
    const peer = new RTCPeerConnection(directPeerConfiguration);
    ...
    peer.onconnectionstatechange = () => {
      ...
      if (peer.connectionState === 'connected') setStatus('Canal direto estabelecido. Compare o código antes da mídia.');
  ...
  return <main>
    <section className="intro"><p className="eyebrow">SFScreen V0 · P2P direto</p><h1>Teste de conexão sem servidor</h1><p>Troque um convite e uma resposta por arquivo. Esta etapa usa somente candidatos locais: sem STUN, TURN, VPN ou backend.</p></section>
    <section className="workspace" aria-label="Sessão direta SFScreen">
      <section className="card"><h2>1. Apresentador</h2><p>Crie e envie o arquivo <code>.sfsinvite</code> para o espectador.</p><button className="primary" type="button" onClick={() => void createInvite()} disabled={role !== undefined}>Criar convite</button></section>
